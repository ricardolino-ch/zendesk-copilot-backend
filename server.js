...
function promptFor({ action, targetLanguage, text, requesterName, ticketContext }) {
  ...
  if (action === "reply_from_summary") return `You are a Ricardo support agent. Write a short, clear, friendly customer reply in ${language} from this internal summary. The reply MUST be entirely in ${language}; do not default to German. Do not expose internal wording or invent facts. Use exactly this greeting:\n${getGreeting(targetLanguage, requesterName)}\n\nUse exactly this closing:\n${getClosing(targetLanguage)}\n\nInternal summary:\n${text}`;
