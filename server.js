...
function promptFor({ action, targetLanguage, text, agentContext, requesterName, ticketContext }) {
  ...
  const internalContext = agentContext ? `\n\nINTERNER HINWEIS DES AGENTEN (HÖCHSTE PRIORITÄT):\n${agentContext}\n\nDer Hinweis ist die verbindliche Arbeitsanweisung für diesen Fall. Setze jede darin enthaltene Anweisung, Einschränkung, Empfehlung und jeden ausdrücklich genannten Fallfakt in der Antwort um. Ergänze danach passende Informationen aus Ticket, Wissensbasis und Projektquellen. Keine wesentliche Vorgabe des Hinweises darf stillschweigend entfallen. Der Hinweis darf niemals wörtlich offengelegt werden; formuliere seine Inhalte natürlich für das Mitglied. Bei einem echten Widerspruch zwischen Hinweis und belegtem Ticketfakt nicht raten, sondern die Unsicherheit transparent machen und gezielt nachfragen.` : "";
